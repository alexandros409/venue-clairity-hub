...
export const analyzeBottleneck = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<Analysis> => {
    ...
    const stepsInstruction = (() => {
      if (obsType === "positive") {
        return [
          "Πρόκειται για POSITIVE observation (καλή πρακτική που αξίζει να διατηρηθεί) — ΟΧΙ πρόβλημα.",
          "Δώσε ακριβώς 3 σύντομες, αυθεντικές προτάσεις στα Ελληνικά που:",
          "  (1) επιβραβεύουν συγκεκριμένα (όχι γενικά) αυτό που έγινε καλά,",
          "  (2) προτείνουν πώς να αποτυπωθεί ως πρότυπο για την ομάδα,",
          "  (3) καλούν την ομάδα να το επαναλάβει με τον ίδιο ρυθμό στην επόμενη βάρδια.",
          "Απαγορεύονται λέξεις όπως «διορθώστε», «πρόβλημα», «αδυναμία», «fix». Είναι ΜΟΝΟ αναγνώριση.",
        ].join(" ");
      }
      ...
      return [
        ...
        "Δώσε ακριβώς 3 σύντομες προτάσεις στα Ελληνικά, σαν mentor που μιλάει σε νέο σερβιτόρο.",
        "Η κάθε πρόταση να αγγίζει τόσο τη ΔΟΜΗ (τι λείπει από το σύστημα) όσο και το ΣΥΝΑΙΣΘΗΜΑ (πώς το ζει ο πελάτης / η ομάδα).",
        "Ξεκίνα με ζεστές εκφράσεις όπως «Θυμήσου ότι…», «Το μυστικό εδώ είναι…», «Ένα απλό βλέμμα και ένα χαμόγελο…».",
      ].join(" ");
    })();
